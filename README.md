...
| Classification | MobileNetV3-Large (Fine-tuned) | The final model for severity prediction. |
| API/Integration | Fast API / Python | Used for creating the REST API service for model deployment. |
...
- **Cropping and Resizing**: After detection and segmentation, images are subjected to precise cropping (using contour detection) and subsequently resized to a uniform dimension (e.g., 224×224 pixels). This step is essential for input consistency and model performance.
...
### Step 1: Segmentation Model (U-Net Architecture)
